...
export function Feed({type = "feed", showPostBox = true, context = null, did = null, autoRefresh = false, encryptionRules = null}) {
  ...
  const [domainHolder, setDomainHolder] = useState(false); // is user a domain holder (non-holders do not see the post box)
  ...
  async function checkIfDomainHolder() {
    const userStoredDomain = await sessionStorage.getItem(address);

    if (userStoredDomain && String(userStoredDomain).includes(".")) {
      setDomainHolder(true);
    } else {
      setDomainHolder(false);
    }
  }
